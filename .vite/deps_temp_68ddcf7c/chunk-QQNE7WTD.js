import { sha256 } from "./chunk-DSWFES7F.js";
import {
  Hash,
  bytes,
  concatBytes,
...
import { __export } from "./chunk-256EKJAK.js";
...
function aInRange(title, n, min, max) {
  if (!inRange(n, min, max))
    throw new Error(
      `expected valid ${title}: ${min} <= n < ${max}, got ${typeof n} ${n}`
    );
}
function bitLen(n) {
  let len;
...
function pow(num2, power, modulo) {
  ...
  let res = _1n2;
  while (power > _0n2) {
    if (power & _1n2) res = (res * num2) % modulo;
...
function Field(ORDER, bitLen2, isLE = false, redef = {}) {
  ...
  const f = Object.freeze({
    ...
    cmov: (a, b, c) => (c ? b : a),
    toBytes: (num2) =>
      isLE ? numberToBytesLE(num2, BYTES) : numberToBytesBE(num2, BYTES),
    fromBytes: (bytes2) => {
...
var DER = {
  ...
  hexFromSig(sig) {
    ...
    const seq = `${tlv.encode(2, int.encode(sig.r))}${tlv.encode(
      2,
      int.encode(sig.s)
    )}`;
...
function weierstrass(curveDef) {
  ...
  const numToNByteStr = (num2) =>
    bytesToHex(numberToBytesBE(num2, CURVE.nByteLength));
  ...
  function verify(signature, msgHash, publicKey, opts = defaultVerOpts) {
    ...
    const u2 = modN2(r * is);
    const R =
      (_a = Point2.BASE.multiplyAndAddUnsafe(P, u1, u2)) == null
        ? void 0
        : _a.toAffine();
    if (!R) return false;
    const v = modN2(R.x);
...
export { secp256k1, schnorr, hashToCurve, encodeToCurve };
...
@noble/curves/esm/secp256k1.js:
  (*! noble-curves - MIT License (c) 2022 Paul Miller (paulmillr.com) *)
*/
//# sourceMappingURL=chunk-QQNE7WTD.js.map
